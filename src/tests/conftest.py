...
def pytest_addoption(parser):
    parser.addoption(
        ...
        action="store",
        default="local",
        help="Which environment to use to run tests on",
        choices=("local", "saucelabs"),
    )

    parser.addoption("--dc", 
                     action="store", 
                     default='us', 
                     help="Set Sauce Labs Data Center (US or EU)"
                     )
...
def driver(request) -> webdriver:
    ...
    elif test_env == "saucelabs":
        return mobile_web_driver
    return driver
